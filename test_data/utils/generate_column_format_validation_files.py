...
column_formats = {
    "transactions_2022.csv": ["Transaction_ID", "Amount", "Transaction_Date", "Customer_Name"],
    "transactions_2023.xlsx": ["Transaction_Code", "Transaction_Date", "Reference"],
    "transactions_numeric.csv": ["Transaction_ID", "Amount", "Transaction_Date", "Description"],
    "transactions_text.xlsx": ["Customer_Name", "Transaction_Date"],
    "batch_numeric.csv": ["Transaction_ID", "Amount", "Transaction_Date", "Status"],
    "batch_alphanumeric.csv": ["Transaction_Code", "Reference", "Transaction_Date", "Status"],
    "batch_mixed.csv": ["Transaction_ID", "Amount", "Reference", "Transaction_Date", "Status"],
    "transactions_2019.csv": ["Transaction_ID", "Amount", "Transaction_Date", "Invalid_Column"],
    "transactions_2021.xlsx": ["Transaction_Code", "Transaction_Date", "Reference"],
    "performance_2015_2020.csv": ["Transaction_ID", "Amount", "Transaction_Date", "Status"],
    "performance_2021_2023.parquet": ["Transaction_ID", "Amount", "Transaction_Date", "Status"]
}
...
def generate_test_file(file_name, columns):
    ...
    data = {
        ...
        "Description": ["Payment", "Withdrawal", "Deposit", "Fee"] * (
                    row_count // 4) if "Description" in columns else None,
        "Status": ["Completed", "Pending", "Failed", "Completed"] * (row_count // 4) if "Status" in columns else None,
        "Invalid_Column": ["#ERROR", "@INVALID", "$FAIL", "&CORRUPT"] * (
                    row_count // 4) if "Invalid_Column" in columns else None,
    }
