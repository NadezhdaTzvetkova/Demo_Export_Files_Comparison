...
def generate_transaction_data(file_name, transaction_count, has_duplicates):
    ...
    if has_duplicates:
        duplicate_indices = np.random.choice(
            transaction_count, size=int(transaction_count * 0.001), replace=False
        )
        transaction_references[duplicate_indices] = transaction_references[
            np.random.choice(duplicate_indices)
        ]
