...
header_mismatch_scenarios = {
    "transactions_wrong_headers.csv": ["Amount", "Transaction_ID", "Transaction_Date", "Customer_Name"],  # Wrong order
    "transactions_missing_headers.xlsx": ["Transaction_ID", "Transaction_Date"],  # Missing columns
    "transactions_case_mismatch.csv": ["transaction_id", "amount", "transaction_date", "customer_name"],  # Wrong case
    "transactions_data_type_mismatch.xlsx": ["Transaction_ID", "Amount (Text)", "Transaction_Date", "Customer_Name"],  # Data type issue
}
...
database_mismatch_files = {
    "transactions_reordered_headers.csv": ["Customer_Name", "Transaction_Date", "Transaction_ID", "Amount"],
    "transactions_duplicate_headers.xlsx": ["Transaction_ID", "Amount", "Amount", "Transaction_Date"],
}
...
batch_processing_headers = ["Transaction_ID", "Amount", "Transaction_Date", "Extra_Header"]
...
error_handling_files = {
    "transactions_2019.csv": ["Unexpected_Header", "Transaction_ID", "Amount", "Transaction_Date"],
    "transactions_2021.xlsx": ["Misaligned_Column1", "Transaction_ID", "Amount", "Transaction_Date"],
    "transactions_2023_with_errors.csv": ["Transaction_ID", "Amount", "Transaction_Date", "Extra_Header1", "Extra_Header2"],
}
...
performance_headers = ["Transaction_ID", "Amount", "Transaction_Date", "Status", "Extra_Header"]
performance_data_1 = {
...
df_performance_1.to_csv(os.path.join(SAVE_DIR, "performance_2015_2020.csv"), index=False)

# ✅ Handle missing pyarrow for Parquet
try:
    import pyarrow
    df_performance_2.to_parquet(os.path.join(SAVE_DIR, "performance_2021_2023.parquet"), index=False, engine="pyarrow")
except ImportError:
    print("⚠️ Warning: pyarrow not installed. Saving performance_2021_2023 as CSV instead.")
    df_performance_2.to_csv(os.path.join(SAVE_DIR, "performance_2021_2023.csv"), index=False)
