...
def generate_test_file(file_name, columns):
    ...
    data = {
        ...
        "Extra_Column1": (
            ["ExtraData1"] * row_count if "Extra_Column1" in columns else None
        ),
        "Extra_Column2": (
            ["ExtraData2"] * row_count if "Extra_Column2" in columns else None
        ),
        "Extra_Column": (
            ["ExtraInfo"] * row_count if "Extra_Column" in columns else None
        ),
        "Unexpected_Field": (
            ["#ERROR", "@INVALID", "$FAIL", "&CORRUPT"] * (row_count // 4)
            if "Unexpected_Field" in columns
            else None
        ),
        "Unknown_Column": (
            ["UNKNOWN_" + str(i) for i in range(row_count)]
            if "Unknown_Column" in columns
            else None
        ),
    }
