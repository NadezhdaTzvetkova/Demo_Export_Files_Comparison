...
def attach_image_binary(context, image_binary, header_text=None):
    ...
    try:
        ...
        if image_binary_format not in ["PNG", "JPEG"]:
            logging.error(
                "[behavex-images] The provided binary data is not a valid PNG or JPG image."
            )
            return
        ...
        logging.error(
            "[behavex-images] The provided binary is not a valid image, or could not be converted to PNG: %s"
            % str(exception)
        )
        return
    try:
        if (
            not context.bhximgs_image_hash
            or image_stream_hash != context.bhximgs_image_hash
        ):
        ...
        if not context.bhximgs_log_stream.closed:
            if header_text:
                context.bhximgs_previous_steps.append(
                    normalize_log(header_text, line_breaks=2)
                )
        ...
        logging.error(
            "[behavex-images] It was not possible to add the image to the report: %s"
            % str(exception)
        )
...
def attach_image_file(context, file_path, header_text=None):
    ...
    if os.path.isfile(file_path):
        ...
        if file_extension.lower() not in [".jpg", ".png"]:
            logging.error(
                "[behavex-images] The provided file format is not supported. Only PNG and JPG files can be attached."
            )
            return
        ...
        logging.error(
            "[behavex-images] The provided file cannot be found at the specified path:  %s"
            % file_path
        )
