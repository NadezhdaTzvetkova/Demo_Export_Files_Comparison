...
def copy_bootstrap_html_generator(output):
    ...
    retry_file_operation(dest_path, lambda: shutil.copytree(bootstrap_path, dest_path))
...
def create_log_path(name, execution_retry=False):
    ...
    while os.path.exists(path):
        if execution_retry:
            return path
        path = initial_path + "_" + str(scenario_outline_index)
        scenario_outline_index += 1
    try:
        os.makedirs(path)
...
def get_error_message(error_message):
    ...
    formatted_error = "\n".join(
        [16 * " " + line for line in text(error_message).split("\n")]
    ).strip()
...
def get_environment_details():
    ...
    environment_details = (
        environment_details_raw_data.split(",") if environment_details_raw_data else []
    )
