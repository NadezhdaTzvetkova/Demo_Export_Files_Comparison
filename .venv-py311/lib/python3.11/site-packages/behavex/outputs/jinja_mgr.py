...
def get_path_extra_logs(scenario):
    extra_logs_folder = os.path.join(
        ...
        str(
            get_string_hash(
                "{}-{}".format(str(scenario["filename"]), str(scenario["line"]))
            )
        ),
        "evidence",
    )
...
def get_lines_exception(step):
    if type(step) is dict:
        return "\n".join([16 * " " + line for line in step["error_lines"]]).strip()
    elif step.exception:
        return "\n".join(
            [16 * " " + line for line in traceback.format_tb(step.exc_traceback)]
        ).strip()
    else:
        return ""
