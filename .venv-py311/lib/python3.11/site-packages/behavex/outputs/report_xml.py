...
def _export_feature_to_xml(feature, isobject=True):
    ...
    scenarios = [
        ...
        if (
            match_for_execution(get_scenario_tags(scenario))
            and not (
                get_status(scenario) == Status.skipped and get_env("RERUN_FAILURES")
            )
        )
    ]

    skipped = [
        scenario for scenario in scenarios if get_status(scenario) == Status.skipped
    ]
    failures = [
        scenario
        for scenario in scenarios
        if get_status(scenario) == Status.failed
        or get_status(scenario) == Status.untested
    ]
    muted = [
        scenario
        for scenario in scenarios
    ...
    muted_failed = [
        scenario for scenario in muted if get_status(scenario) == Status.failed
    ]
    ...
    name = ".".join(exp.split(filename.partition("features")[2][1:-8]))
    junit_path = os.path.join(get_env("OUTPUT"), "behave")
    path_output = os.path.join(junit_path, "TESTS-" + name + ".xml")
