...
def setup_running_failures(args_parsed):
    ...
    if failures_path:
        ...
        if os.path.exists(failures_path):
            ...
            print(
                "\nThe specified failing scenarios filename was not found: {}".format(
                    failures_path
                )
            )
            return EXIT_ERROR, None
...
def launch_behavex():
    ...
    totals = {
        "features": {"passed": 0, "failed": 0, "skipped": 0, "untested": 0},
    ...
    try:
        ...
        if isinstance(execution_codes, list):
            ...
            execution_interrupted_or_crashed = (
                True if any([code == 2 for code in execution_codes]) else False
            )
        else:
        ...
        exit_code = (
            EXIT_ERROR
            if (execution_failed and failing_non_muted_tests)
            or execution_interrupted_or_crashed
            else EXIT_OK
        )
        ...
        try:
            while active_children():
                for (
                    process
                ) in active_children():  # Terminate all active child processes
    ...
    if multiprocess:
        ...
        untested_features_msg = (
            ", {} untested".format(untested_features) if untested_features > 0 else ""
        )
        untested_scenarios_msg = (
            ", {} untested".format(untested_scenarios) if untested_scenarios > 0 else ""
        )
        ...
        print(
            "\n{} feature{} passed, {} failed, {} skipped{}".format(
                totals["features"]["passed"],
        ...
        print(
            "{} scenario{} passed, {} failed, {} skipped{}".format(
                totals["scenarios"]["passed"],
        ...
        print(
            "Took: {}".format(
                pretty_print_time(
                    global_vars.execution_end_time - global_vars.execution_start_time
                )
            )
        )
    if results and results["features"]:
        print(
            "\nHTML output report is located at: {}".format(
                os.path.join(get_env("OUTPUT"), "report.html")
            )
        )
            ...
            if global_vars.rerun_failures or ".feature:" in feature_path:
                ...
                if isinstance(scenario, ScenarioOutline):
                    for scenario_outline_instance in scenario.scenarios:
                        outline_scenario_line = get_feature_and_scenario_line(
                            scenario_outline_instance.name
                        )[1]
                        if (
                            outline_scenario_line
                            and scenario_outline_instance.line
                            == int(outline_scenario_line)
                        ):
                            if (
                                scenario_outline_instance
                                not in updated_features[pure_feature_path]
                            ):
                                updated_features[feature_path].append(
                                    scenario_outline_instance
                                )
                            break
                else:
                ...
                if isinstance(scenario, ScenarioOutline):
                    for scenario_outline_instance in scenario.scenarios:
                        if (
                            scenario_outline_instance
                            not in updated_features[updated_features_path]
                        ):
                            updated_features[updated_features_path].append(
                                scenario_outline_instance
                            )
                else:
...
def launch_by_feature(features, process_pool, lock, show_progress_bar):
    ...
    for features_path in features:
        ...
        feature_filename = (
            ...
            else "{}:{}".format(pure_feature_path, scenario_line)
        )
    ...
    if show_progress_bar:
        ...
        global_vars.progress_bar_instance = _get_progress_bar_instance(
            parallel_scheme="feature", total_elements=total_features
        )
    ...
    if serial_features:
        ...
        for serial_feature in serial_features:
            execution_code, map_json = execute_tests(
                features_path=None,
...
def launch_by_scenario(
    features, process_pool, lock, shared_removed_scenarios, show_progress_bar
):
    ...
    for features_path, scenarios in features.items():
        ...
        for scenario in scenarios_instances:
            if include_path_match(
                scenario.filename, scenario.line
                ...
                if match_for_execution(scenario_tags):
                    ...
                    total_scenarios_to_run[feature_filename] = (
                        total_scenarios_to_run.setdefault(feature_filename, 0) + 1
                    )
                    if "SERIAL" in scenario_tags:
                        for key, list_scenarios in serial_scenarios.items():
                            if scenario_information in list_scenarios:
                                duplicated_scenarios.setdefault(key, []).append(
                                    scenario.name
                                )
                        serial_scenarios.setdefault(features_path, []).append(
                            scenario_information
                        )
                    else:
                        ...
                        parallel_scenarios.setdefault(features_path, []).append(
                            scenario_information
                        )
    if show_progress_bar:
        global_vars.progress_bar_instance = _get_progress_bar_instance(
    ...
    if serial_scenarios:
        ...
        for features_path, scenarios_in_feature in serial_scenarios.items():
            for scen_info in scenarios_in_feature:
                scenarios_to_run_in_feature = total_scenarios_to_run[
                    scen_info["feature_filename"]
                ]
                execution_code, json_report = execute_tests(
    ...
    if parallel_scenarios:
        ...
        for features_path in parallel_scenarios.keys():
            for scenario_information in parallel_scenarios[features_path]:
                scenarios_to_run_in_feature = total_scenarios_to_run[
                    scenario_information["feature_filename"]
                ]
                ...
                future.add_done_callback(
                    create_execution_complete_callback_function(
                        execution_codes, json_reports, global_vars.progress_bar_instance
                    )
                )
        for parallel_process in parallel_processes:
            parallel_process.result()
...
def execute_tests(
    ...
    try:
        ...
        try:
            ...
            behave_args = _set_behave_arguments(
                features_path=features_path,
        ...
        if generate_report:
                        ...
                        if scenario_line:
                            for skeleton_scenario in skeleton_feature["scenarios"]:
                                if str(skeleton_scenario["line"]) == str(scenario_line):
                                    ...
                                    skeleton_scenario["error_msg"] = get_text(
                                        "scenario.execution_crashed"
                                    )
                        else:
                            skeleton_feature["status"] = "failed"
                            skeleton_feature[
                                "error_msg"
                            ] = "Execution crashed. No outputs could be generated."
                            for skeleton_scenario in skeleton_feature["scenarios"]:
                                skeleton_scenario["status"] = "failed"
                                skeleton_scenario["error_msg"] = get_text(
                                    "feature.execution_crashed"
                                )
                else:
                    json_output = {
                        "environment": [],
            ...
            if scenario_line:
                json_output["features"] = filter_feature_executed(
                    json_output, text(feature_filename), scenario_line=scenario_line
                )
                if (
                    len(json_output["features"]) == 0
                    or len(json_output["features"][0]["scenarios"]) == 0
                ):
                ...
                try:
                    ...
                    logging.exception(
                        "There was a problem processing the xml file: {}".format(ex)
                    )
        else:
...
def _launch_behave(behave_args):
    ...
    if stdout_file and os.path.exists(stdout_file):
        ...
        def _merge_and_remove():
            with open(
                os.path.join(get_env("OUTPUT"), "merged_behave_outputs.log"), "a+"
            ) as behave_log_file:
...
def wrap_up_process_pools(process_pool, json_reports, scenario=False):
    """
...
def remove_temporary_files(parallel_processes, json_reports):
    ...
    for i in range(parallel_processes):
        ...
        path_stdout = os.path.join(
            gettempdir(), "{}_stdout{}.txt".format(os.getpid(), i + 1)
        )
    ...
    for json_report in json_reports:
        if "features" in json_report and json_report["features"]:
            feature_name = os.path.join(
                get_env("OUTPUT"), "{}.tmp".format(json_report["features"][0]["name"])
            )
...
def processing_xml_feature(
    ...
    if lock:
        lock.acquire()
    try:
        feature_contains_scenarios = (
            True
            if (
                json_output["features"]
                and "scenarios" in json_output["features"][0]
                and json_output["features"][0]["scenarios"]
            )
            else False
        )
        if (
            type(shared_removed_scenarios) == DictProxy
            and not feature_contains_scenarios
        ):
        ...
        if feature_contains_scenarios:
            ...
            feature_name = os.path.join(
                get_env("OUTPUT"), "{}.tmp".format(os.path.basename(feature_filename))
            )
                    ...
                    try:
                        ...
                        with open(feature_name, "w") as feature_file:
                            for executed_scenario in executed_scenarios:
                                processed_feature_data["scenarios"].append(
                                    executed_scenario
                                )
            ...
            if (
                shared_removed_scenarios
                and feature_filename in shared_removed_scenarios
            ):
            ...
            if len(processed_feature_data["scenarios"]) == total_scenarios:
                try:
                    ...
                    path_tmp = "{}.tmp".format(feature_name[:-4])
                    os.remove(path_tmp)
    except Exception as ex:
        raise ex
    finally:
...
def _set_behave_arguments(
    ...
    if multiprocess:
        ...
        updated_features_path = (
            updated_features_path
            if not scenario_line
            else "{}:{}".format(updated_features_path, scenario_line)
        )
        ...
        # Appending the process ID to the stdout file as a prefix, to avoid conflicts with other BehaveX processes
        arguments.append(
            os.path.join(gettempdir(), "{}_stdout{}.txt".format(os.getpid(), worker_id))
        )
...
def _get_progress_bar_instance(parallel_scheme, total_elements):
    ...
    try:
        config = conf_mgr.get_config()
        print_updates_in_new_lines = (
            True if config["progress_bar"]["print_updates_in_new_lines"] else False
        )
        progress_bar_prefix = "Executed {}s".format(parallel_scheme)
        progress_bar_instance = ProgressBar(
            progress_bar_prefix,
            total_elements,
