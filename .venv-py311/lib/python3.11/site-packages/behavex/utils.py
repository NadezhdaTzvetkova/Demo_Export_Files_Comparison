...
def handle_execution_complete_callback(
    codes, json_reports, progress_bar_instance, future
):
    tuple_values = None
    try:
...
def create_execution_complete_callback_function(
    ...
    append_output = functools.partial(
        handle_execution_complete_callback, codes, json_reports, progress_bar_instance
    )
...
def join_feature_reports(json_reports):
    if type(json_reports) is list:
        if len(json_reports) == 1:
            ...
            merged_json["features"] = sum(
                (json_["features"] for json_ in json_reports), []
            )
    else:
        merged_json = json_reports
    if merged_json["features"] and (
        IncludeNameMatch().bool() or IncludePathsMatch().bool() or MatchInclude().bool()
    ):
        delete = []
...
def copy_bootstrap_html_generator():
    ...
    if os.path.exists(destination_path):
        retry_file_operation(destination_path, lambda: shutil.rmtree(destination_path))
    retry_file_operation(
...
def print_env_variables(keys):
    ...
    value_length = 60
    separator = "|{}| {}|".format(
        "".ljust(key_length, "-"), "".ljust(value_length, "-")
    )
    header = "|{}| {}|".format(
    ...
    for key in keys:
        ...
        print(
            "|{}| {}|".format(
                key.upper().ljust(key_length), str(value).ljust(value_length)
            )
        )
    print(separator)
...
def get_text(key_chain):
    ...
    for i, key in enumerate(keys):
        ...
        if isinstance(result, dict):
            ...
            return 'key "{}" no found '.format(key_chain)
    if isinstance(result, dict):
...
def create_custom_log_when_called(self, key):
    if key == "evidence_path":
        if not hasattr(self, "log_path"):
            ...
            self.log_path = get_string_hash(
                "{}-{}".format(str(self.feature.filename), str(self.scenario.line))
            )
        ...
        try:
            ...
            print(
                "It was not possible to create the folder to store additional scenario evidence..."
            )
            raise error
...
class MatchInclude(metaclass=ExecutionSingleton):
    ...
    def match(self, filename):
        ...
        return self.reg.match(filename) is not None
...
class IncludePathsMatch(metaclass=ExecutionSingleton):
    def __init__(self, paths=None):
        ...
        self.scenarios = [
            "{}:{}".format(
                os.path.abspath(get_feature_and_scenario_line(path)[0]),
                get_feature_and_scenario_line(path)[1],
            )
            for path in self.features_paths
...
class IncludeNameMatch(metaclass=ExecutionSingleton):
    ...
    def match(self, scenario):
        return self.reg.match(scenario) is not None
