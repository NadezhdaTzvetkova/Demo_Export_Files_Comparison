        ...
        for scenario in scenarios_instances:
            ...
            if get_param("dry_run"):
                if "MANUAL" not in scenario_tags:
                    scenario.tags.append("BHX_MANUAL_DRY_RUN")
                    scenario.tags.append("MANUAL")
...
def before_scenario(context, scenario):
    ...
    try:
        ...
        concat_feature_and_scenario_line = "{}-{}".format(
            str(context.feature.filename), str(scenario.line)
        )
        context.log_path = create_log_path(
            concat_feature_and_scenario_line, retrying_execution
        )
...
def _add_log_handler(log_path):
    ...
    file_handler.addFilter(
        lambda record: setattr(record, "msg", strip_ansi_codes(str(record.msg))) or True
    )
