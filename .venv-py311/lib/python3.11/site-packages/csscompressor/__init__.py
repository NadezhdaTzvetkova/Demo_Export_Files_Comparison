...
_ms_filter_re = re.compile(
    r"progid\:DXImageTransform\.Microsoft\.Alpha\(Opacity=", re.I
)
...
_pseudo_re = re.compile(
    r""":(active|after|before|checked|disabled|empty|enabled|
                                    first-(?:child|of-type)|focus|hover|last-(?:child|of-type)|
                                    link|only-(?:child|of-type)|root|:selection|target|visited)""",
    re.I | re.X,
)

_common_funcs_re = re.compile(
    r""":(lang|not|nth-child|nth-last-child|nth-last-of-type|
                                        nth-of-type|(?:-(?:moz|webkit)-)?any)\(""",
    re.I | re.X,
)

_common_val_funcs_re = re.compile(
    r"""([:,\(\s]\s*)(attr|color-stop|from|rgba|to|url|
                                        (?:-(?:atsc|khtml|moz|ms|o|wap|webkit)-)?
...
_zero_fmt_spec_re = re.compile(
    r"""(\s|:|\(|,)(?:0?\.)?0
                                    (?:px|em|%|in|cm|mm|pc|pt|ex|deg|g?rad|k?hz)""",
    re.I | re.X,
)

_zero_req_unit_re = re.compile(
    r"""(\s|:|\(|,)(?:0?\.)?0
                                    (m?s)""",
    re.I | re.X,
)

_bg_pos_re = re.compile(
    r"""(background-position|webkit-mask-position|transform-origin|
...
_border_re = re.compile(
    r"""(border|border-top|border-right|border-bottom|
...
_ie_matrix_re = re.compile(
    r"\s*filter:\s*progid:DXImageTransform\.Microsoft\.Matrix\(([^\)]+)\);"
)
...
_colors_re = re.compile(
    r"(:|\s)" + "(\\#(" + "|".join(_colors_map.keys()) + "))" + r"(;|})", re.I
)
...
def _preserve_call_tokens(css, regexp, preserved_tokens, remove_ws=False):
    ...
    for match in regexp.finditer(css):
        ...
        while not found_term and (end_idx + 1) <= max_idx:
            ...
            if end_idx > 0:
                if nest_idx > 0 and nest_idx < end_idx and css[nest_idx - 1] != "\\":
                    nested = True
        ...
        preserver = "{0}(___YUICSSMIN_PRESERVED_TOKEN_{1}___)".format(
            name, len(preserved_tokens)
        )
    ...
    while True:
        ...
        if match.group(1):
            ...
            if (
                ...
                and match.group(4).lower() == match.group(5).lower()
                and match.group(6).lower() == match.group(7).lower()
            ):
                buf.append(
                    "#" + (match.group(2) + match.group(4) + match.group(6)).lower()
                )
...
def _compress(css, max_linelen=0, preserve_exclamation_comments=True):
    ...
    def _replace(match):
        ...
        if "___YUICSSMIN_PRESERVE_CANDIDATE_COMMENT_" in token:
            token = _yui_comment_re.sub(
                lambda match: comments[int(match.group("num"))], token
            )
    ...
    for i, token in enumerate(comments_iter):
        ...
        if token.endswith("\\"):
            ...
            preserved_tokens.append("")
            css = css.replace(
                "___YUICSSMIN_PRESERVE_CANDIDATE_COMMENT_{0}___".format(i + 1),
                "___YUICSSMIN_PRESERVED_TOKEN_{0}___".format(len(preserved_tokens) - 1),
            )
        ...
        if not token:
            ...
            if start_idx > 2:
                if css[start_idx - 3] == ">":
                    ...
                    css = css.replace(
                        placeholder,
                        "___YUICSSMIN_PRESERVED_TOKEN_{0}___".format(
                            len(preserved_tokens) - 1
                        ),
                    )
    ...
    css = _spaces1_re.sub(
        lambda match: match.group(0).replace(":", "___YUICSSMIN_PSEUDOCLASSCOLON___"),
        css,
    )
    ...
    css = _charset1_re.sub(
        lambda match: match.group(2).lower() + " " + match.group(3) + match.group(1),
        css,
    )

    # When all @charset are at the top, remove the second and after (as they are completely ignored)
    css = _charset2_re.sub(
        lambda match: match.group(2) + match.group(3).lower() + " " + match.group(4),
        css,
    )
    ...
    css = _common_val_funcs_re.sub(
        lambda match: match.group(1) + match.group(2).lower(), css
    )
    ...
    css = _zero_req_unit_re.sub(
        lambda match: match.group(1) + "0" + match.group(2), css
    )
    ...
    css = _bg_pos_re.sub(
        lambda match: match.group(1).lower() + ":0 0" + match.group(2), css
    )
    ...
    css = _colors_re.sub(
        ...
        + _colors_map[match.group(3).lower()]
        + match.group(4),
        css,
    )
    ...
    css = _border_re.sub(
        lambda match: match.group(1).lower() + ":0" + match.group(2), css
    )
...
def compress(css, max_linelen=0, preserve_exclamation_comments=True):
    ...
    css, preserved_tokens = _compress(
        css,
        max_linelen=max_linelen,
        preserve_exclamation_comments=preserve_exclamation_comments,
    )
...
def compress_partitioned(
    css, max_linelen=0, max_rules_per_file=4000, preserve_exclamation_comments=True
):
