...
class TestYUI(BaseTest):
    ...
    def test_yui_background_position(self):
        ...
        output = """a{background-position:0 0}b{background-position:0 0}"""

        self._test(input, output)

    def test_yui_border_none(self):
        input = """
            a {
                border: none;
            }
        ...
        output = """a{border:0}b{background:0}s{border-top:0;border-right:0;border-bottom:0;border-left:0}"""

        self._test(input, output)

    def test_yui_box_model_hack(self):
        input = """
            #elem {
             width: 100px;
             voice-family: "\"}\"";
        ...
        output = """#elem{width:100px;voice-family:"\"}\"";voice-family:inherit;width:200px}html>body #elem{width:200px}"""

        self._test(input, output)

    def test_yui_bug2527974(self):
        input = """
        ...
        output = """/*! $LastChangedRevision: 81 $ $LastChangedDate: 2009-05-27 17:41:02 +0100 (Wed, 27 May 2009) $ */body{yo:cats}ul[id$=foo] label:hover{yo:yo}"""

        self._test(input, output)

    def test_yui_bug2527991(self):
        input = """
        ...
        output = """@media screen and/*!YUI-Compresser */(-webkit-min-device-pixel-ratio:0){a{b:1}}@media screen and/*! *//*! */(-webkit-min-device-pixel-ratio:0){a{b:1}}@media -webkit-min-device-pixel-ratio:0{a{b:1}}"""

        self._test(input, output)

    def test_yui_bug2527998(self):
        input = """
            /*! special */
            body {
    ...
    def test_yui_bug2528034(self):
        ...
        output = """a[href$="/test/"] span:first-child{b:1}"""

        self._test(input, output)

    def test_yui_charset_media(self):
        input = """
            /* re: 2495387 */
            @charset 'utf-8';
            @media all {
        ...
        output = """@charset 'utf-8';@media all{body{background-color:gold}}"""

        self._test(input, output)

    def test_yui_color_keyword(self):
        input = """
        ...
        output = """.c1{color:red}.c2{color:navy}.c3{color:gray}.c4{color:olive}.c5{color:purple}.c6{color:silver}.c7{color:teal}.c8{color:orange}.c9{color:maroon}"""

        self._test(input, output)

    def test_yui_color_simple(self):
        input = """
            .foo, #AABBCC {
        ...
        output = """.foo,#AABBCC{background-color:#abc;border-color:#e6a #abcdef #feab2c;filter:chroma(color = #FFFFFF);filter:chroma(color="#AABBCC");filter:chroma(color='#BBDDEE');color:#123}"""

        self._test(input, output)

    def test_yui_color(self):
        input = """
            .color {
              me: rgb(123, 123, 123);
    ...
    def test_yui_concat_charset(self):
        input = """
        ...
        output = """@charset "utf-8";#foo{border-width:1px}#bar{border-width:10px}"""

        self._test(input, output)

    def test_yui_dataurl_base64_doublequotes(self):
        input = """
    ...
    def test_yui_dataurl_base64_noquotes(self):
        input = """
            .yui3-skin-night .yui3-dial-ring-vml,
            .yui3-skin-night .yui3-dial-center-button-vml,
            .yui3-skin-night .yui3-dial-marker v\:oval.yui3-dial-marker-max-min,
    ...
    def test_yui_dataurl_base64_singlequotes(self):
        input = """
            .yui3-skin-night .yui3-dial-ring-vml,
            .yui3-skin-night .yui3-dial-center-button-vml,
            .yui3-skin-night .yui3-dial-marker v\:oval.yui3-dial-marker-max-min,
    ...
    def test_yui_dataurl_base64_twourls(self):
        input = """
            .yui3-skin-night .yui3-dial-ring-vml,
            .yui3-skin-night .yui3-dial-center-button-vml,
            .yui3-skin-night .yui3-dial-marker v\:oval.yui3-dial-marker-max-min,
    ...
    def test_yui_dataurl_dbquote_font(self):
        input = """
    ...
    def test_yui_dataurl_nonbase64_doublequotes(self):
        ...
        output = """div.nonbase64-doublequotes{width:100px;height:100px;background-image:url("data:image/png,%89PNG%0D%0A%1A%0A%00%00%00%0DIHDR%00%00%00%0C%00%00%00%0E%08%03%00%00%00%2Cc%0D%DE%00%00%00%A2PLTEQQQ%FA%FA%FA%FC%FC%FC%EE%EE%EE%A9%A9%A9%E9%E9%E9%0A%0A%0A%0D%0D%0D444PPP%CD%CD%CD%CC%CC%CC%F5%F5%F5UUU%D0%D0%D0'''%F9%F9%F9%A6%A6%A6%40%40%40FFF%A0%A0%A0%89%89%89%8D%8D%8D%20%20%20%14%14%14%DA%DA%DA%B6%B6%B6%02%02%02%87%87%87%81%81%81%AC%AC%AC%0E%0E%0E111%7D%7D%7D%92%92%92333%B9%B9%B9%BC%BC%BChhh)))%E1%E1%E1%03%03%03%CB%CB%CB%EB%EB%EB%FD%FD%FD%A3%A3%A3(((%04%04%04%CA%CA%CAttt%2C%2C%2C%F4%F4%F4%00%00%00%FF%FF%FF%D6%DE%02%C3%00%00%006tRNS%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%00%A1%8FN1%00%00%00iIDAT%08%D7E%C7E%16%02Q%10%C5%D0j%C3%DD%BDqw%5E%F6%BF5%06%D4%3Fd%94kx%BDf%DE6%FFIA%AB%C8qYj%1F%E3Xk%93%E0%C8JZ%10%90%9E%3A1%60%BBY%85%A8%AE%14%C0%5E%1A6%8E%C5w%02%60%99%C9%FA%9A%03%60%8C%EFz%8C%CE%0EnSu%3F%01%AD%B2%06%04%F0%3CT%FF%B8nk%3F%7C%01%C5z%1B%F9%26%2F%3Az%00%00%00%00IEND%AEB%60%82");border:1px solid #0a0}span.othercss{font-family:"Times New Roman";font-weight:inherit}"""

        self._test(input, output)

    def test_yui_dataurl_nonbase64_noquotes(self):
        input = """
        ...
        output = """div.nonbase64-noquotes{width:100px;height:100px;background-image:url(data:image/png,%89PNG%0D%0A%1A%0A%00%00%00%0DIHDR%00%00%00%0C%00%00%00%0E%08%03%00%00%00%2Cc%0D%DE%00%00%00%A2PLTEQQQ%FA%FA%FA%FC%FC%FC%EE%EE%EE%A9%A9%A9%E9%E9%E9%0A%0A%0A%0D%0D%0D444PPP%CD%CD%CD%CC%CC%CC%F5%F5%F5UUU%D0%D0%D0\'\'\'%F9%F9%F9%A6%A6%A6%40%40%40FFF%A0%A0%A0%89%89%89%8D%8D%8D%20%20%20%14%14%14%DA%DA%DA%B6%B6%B6%02%02%02%87%87%87%81%81%81%AC%AC%AC%0E%0E%0E111%7D%7D%7D%92%92%92333%B9%B9%B9%BC%BC%BChhh\)\)\)%E1%E1%E1%03%03%03%CB%CB%CB%EB%EB%EB%FD%FD%FD%A3%A3%A3\(\(\(%04%04%04%CA%CA%CAttt%2C%2C%2C%F4%F4%F4%00%00%00%FF%FF%FF%D6%DE%02%C3%00%00%006tRNS%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%00%A1%8FN1%00%00%00iIDAT%08%D7E%C7E%16%02Q%10%C5%D0j%C3%DD%BDqw%5E%F6%BF5%06%D4%3Fd%94kx%BDf%DE6%FFIA%AB%C8qYj%1F%E3Xk%93%E0%C8JZ%10%90%9E%3A1%60%BBY%85%A8%AE%14%C0%5E%1A6%8E%C5w%02%60%99%C9%FA%9A%03%60%8C%EFz%8C%CE%0EnSu%3F%01%AD%B2%06%04%F0%3CT%FF%B8nk%3F%7C%01%C5z%1B%F9%26%2F%3Az%00%00%00%00IEND%AEB%60%82);border:1px solid red}span.othercss{font-family:"Times New Roman";font-weight:inherit}"""

        self._test(input, output)

    def test_yui_dataurl_noquote_multiline_font(self):
        input = """
            /*csslint fontfamily: true*/
...
"""
    ...
    def test_yui_dataurl_realdata_doublequotes(self):
        input = """
    ...
    def test_yui_dataurl_realdata_singlequotes(self):
        input = """
            .yui3-skin-sam .yui3-scrollview-scrollbar {
                -webkit-transform: translate3d(0, 0, 0);
                -moz-transform: translate(0, 0);
    ...
    def test_yui_dataurl_realdata_yuiapp(self):
        input = """
            html {
                background: #fff;
                color: #555;
...
*/foo{bar:baz}"""
    ...
    def test_yui_font_face(self):
        ...
        output = """@font-face{font-family:'gzipper';src:url(yanone.eot);src:local('gzipper'),url(yanone.ttf) format('truetype')}"""

        self._test(input, output)

    def test_yui_ie5mac(self):
        input = """
        ...
        output = """/*\*/.selector{color:khaki}/**/"""

        self._test(input, output)

    def test_yui_lowercasing(self):
        input = """
            @CHARSET "UTF-8";
    ...
    def test_yui_media_empty_class(self):
        ...
        output = """/*! preserved */@import "another.css";@media print{.noprint{display:none}}@media screen{.printonly{display:none}}"""

        self._test(input, output)

    def test_yui_media_multi(self):
        input = """
        ...
        output = """@media only all and (max-width:50em),only all and (max-device-width:800px),only all and (max-width:780px){some-css:here}"""

        self._test(input, output)

    def test_yui_media_test(self):
        input = """
            @media screen AND (-webkit-min-device-pixel-ratio:0) {
              some-css : here
            }
    ...
    def test_yui_old_ie_filter_matrix(self):
        input = """
            .a {
    ...
    def test_yui_opacity_filter(self):
        ...
        output = """pre{border:solid red;opacity:.8;-ms-filter:"alpha(opacity=80)";filter:alpha(opacity=80);zoom:1}code{-ms-filter:"alpha(opacity=80)";filter:alpha(opacity=80)}"""

        self._test(input, output)

    def test_yui_opera_pixel_ratio(self):
        input = """
            @media
        ...
        output = """.something{foo:bar}"""

        self._test(input, output)

    def test_yui_preserve_case(self):
        input = """
            #AddAddressForm {
                padding: 0;
            }
        ...
        output = """#AddAddressForm{padding:0}#AddAddressForm .messageBoxNeutral{padding:0}#FeedbackMailForm{padding:0}#FeedbackMailForm .classe{margin:0}.classes,#FeedBackMailForm{margin:0}"""

        self._test(input, output)

    def test_yui_preserve_important(self):
        input = """
            .red { color: red !important; }

        """

        output = """.red{color:red !important}"""

        self._test(input, output)

    def test_yui_preserve_new_line(self):
        input = r"""
#sel-o {
  content: "on\"ce upon \
a time";
...
a ti\'me';
        ...
        output = r"""#sel-o{content:"on\"ce upon \
a time";content:'once upon \
a ti\'me'}"""

        self._test(input, output)

    def test_yui_preserve_strings(self):
        input = r"""
            /* preserving strings */
            .sele {
              content: "\"keep  \"    me";
        ...
        output = r""".sele{content:"\"keep  \"    me";something:'\\\' .     . ';else:'empty{}';content:"/* test */"}"""

        self._test(input, output)

    def test_yui_pseudo_first(self):
        input = """
            /*
        ...
        output = """p:first-letter {buh:hum}p:first-line {baa:1}p:first-line ,a,p:first-letter ,b{color:red}"""

        self._test(input, output)

    def test_yui_pseudo(self):
        input = """
            p :link {
              ba:zinga;;;
              foo: bar;;;
        ...
        output = """p :link{ba:zinga;foo:bar}"""

        self._test(input, output)

    def test_yui_special_comments(self):
        input = """
/*!************88****
 Preserving comments
    as they are
...
serve! */
    ...
    def test_yui_star_underscore_hacks(self):
        ...
        output = """#elementarr{width:1px;*width:3pt;_width:2em}"""

        self._test(input, output)

    def test_yui_string_in_comment(self):
        input = """
            /* te " st */
            a{a:1}
            /*!"preserve" me*/
        ...
        output = """a{a:1}/*!"preserve" me*/b{content:"/**/"}/*\*/c{c:3}/**/"""

        self._test(input, output)

    def test_yui_webkit_transform(self):
        input = """
            c {-webkit-transform-origin: 0 0;}
            d {-MOZ-TRANSFORM-ORIGIN: 0 0 }
        """

        output = """c{-webkit-transform-origin:0 0}d{-moz-transform-origin:0 0}"""

        self._test(input, output)

    def test_yui_zeros(self):
        input = """
            a {
    ...
    def test_yui_dataurl_nonbase64_singlequotes(self):
        input = r"""
            /* Some Comment */
        ...
        output = r"""div.nonbase64-singlequotes{width:100px;height:100px;background-image:url('data:image/png,%89PNG%0D%0A%1A%0A%00%00%00%0DIHDR%00%00%00%0C%00%00%00%0E%08%03%00%00%00%2Cc%0D%DE%00%00%00%A2PLTEQQQ%FA%FA%FA%FC%FC%FC%EE%EE%EE%A9%A9%A9%E9%E9%E9%0A%0A%0A%0D%0D%0D444PPP%CD%CD%CD%CC%CC%CC%F5%F5%F5UUU%D0%D0%D0\'\'\'%F9%F9%F9%A6%A6%A6%40%40%40FFF%A0%A0%A0%89%89%89%8D%8D%8D%20%20%20%14%14%14%DA%DA%DA%B6%B6%B6%02%02%02%87%87%87%81%81%81%AC%AC%AC%0E%0E%0E111%7D%7D%7D%92%92%92333%B9%B9%B9%BC%BC%BChhh)))%E1%E1%E1%03%03%03%CB%CB%CB%EB%EB%EB%FD%FD%FD%A3%A3%A3(((%04%04%04%CA%CA%CAttt%2C%2C%2C%F4%F4%F4%00%00%00%FF%FF%FF%D6%DE%02%C3%00%00%006tRNS%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%FF%00%A1%8FN1%00%00%00iIDAT%08%D7E%C7E%16%02Q%10%C5%D0j%C3%DD%BDqw%5E%F6%BF5%06%D4%3Fd%94kx%BDf%DE6%FFIA%AB%C8qYj%1F%E3Xk%93%E0%C8JZ%10%90%9E%3A1%60%BBY%85%A8%AE%14%C0%5E%1A6%8E%C5w%02%60%99%C9%FA%9A%03%60%8C%EFz%8C%CE%0EnSu%3F%01%AD%B2%06%04%F0%3CT%FF%B8nk%3F%7C%01%C5z%1B%F9%26%2F%3Az%00%00%00%00IEND%AEB%60%82');border:1px solid #00a}span.othercss{font-family:"Times New Roman";font-weight:inherit}"""
