...
def step_impl(context):
    context.result = subprocess.run(
        ["behavex", "--help"], capture_output=True, text=True
    )
...
def step_impl(context):
    ...
    assert (
        "BehaveX - test automation wrapper on top of Behave" in context.result.stdout
    ), "Unexpected output from behavex command"
