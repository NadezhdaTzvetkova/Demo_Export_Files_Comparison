...
def step_impl(context):
    context.output_path = os.path.join(
        "output", "output_{}".format(get_random_number(6))
    )
    execution_args = [
        "behavex",
        os.path.join(
            tests_features_path, "secondary_features", "passing_tests.feature"
        ),
        "-o",
        context.output_path,
    ]
...
def step_impl(context):
    ...
    execution_args = [
        "behavex",
        "-rf",
        os.path.join(tests_features_path, "failing_scenarios.txt"),
        "-o",
        context.output_path,
    ]
    execute_command(context, execution_args)
...
def step_impl(context):
    ...
    execution_args = [
        "behavex",
        os.path.join(tests_features_path, "secondary_features", "rename_tests.feature"),
        "-o",
        context.output_path,
    ]
    execute_command(context, execution_args)
...
def step_impl(context):
    ...
    execution_args = [
        "behavex",
        os.path.join(
            tests_features_path, "secondary_features", "failing_tests.feature"
        ),
        "-o",
        context.output_path,
    ]
...
def step_impl(context, parallel_processes="1", parallel_scheme="scenario"):
    ...
    execution_args = [
        ...
        os.path.join(
            tests_features_path,
            os.path.join(
                tests_features_path, "crashing_features", "crashing_tests.feature"
            ),
        ),
        "-o",
        context.output_path,
...
def step_impl(context):
    ...
    execution_args = [
        "behavex",
        os.path.join(
            tests_features_path, "secondary_features", "skipped_tests.feature"
        ),
        "-o",
        context.output_path,
    ]
...
def step_impl(context):
    ...
    execution_args = [
        "behavex",
        os.path.join(
            tests_features_path, "secondary_features", "untested_tests.feature"
        ),
        "-o",
        context.output_path,
    ]
...
@when(
    'I run the behavex command with "{parallel_processes}" parallel processes and parallel scheme set as "{parallel_schema}"'
)
...
@when(
    'I setup the behavex command with "{parallel_processes}" parallel processes and parallel scheme set as "{parallel_scheme}"'
)
...
@when(
    'I run the behavex command with scenario name "{scenario_name}" and the following scheme, processes and tags'
)
@when(
    'I run the behavex command using "{argument_separator}" separator with the following scheme, processes and tags'
)
@when(
    'I run the behavex command using "{argument_separator}" separator for "{feature_name}" feature with the following scheme, processes and tags'
)
@when(
    'I run the behavex command using "{argument_separator}" separator for "{feature_name}" and "{feature_name_2}" features with the following scheme, processes and tags'
)
def run_command_with_scheme_processes_and_tags(
    context,
    scenario_name=None,
    argument_separator="equal",
    feature_name=None,
    feature_name_2=None,
):
    ...
    if feature_name:
        if feature_name_2:
            feature_path = os.path.join(
                tests_features_path, "secondary_features", feature_name
            )
            feature_path_2 = os.path.join(
                tests_features_path, "secondary_features", feature_name_2
            )
        else:
            feature_path = os.path.join(
                tests_features_path, "secondary_features", feature_name
...
@then(
    'I should see the following behavex console outputs and exit code "{expected_exit_code}"'
)
def step_impl(context, expected_exit_code=None):
    if expected_exit_code is not None:
        assert int(context.result.returncode) == int(
            expected_exit_code
        ), "Behavex exit code is not expected"
    for row in context.table:
        assert (
            row["output_line"] in context.result.stdout
        ), f"Unexpected output: {context.result.stdout}\n\nOutput line not found: {row['output_line']}"
...
def step_impl(context):
    ...
    for message in error_messages:
        assert (
            message not in context.result.stdout.lower()
        ), f"Unexpected output: {context.result.stdout}"
...
def step_impl(context):
    ...
    logging.info(
        f"Total scenarios in the HTML report: {total_scenarios_in_html_report}"
    )
    total_scenarios_in_junit_reports = get_total_scenarios_in_junit_reports(context)
    logging.info(
        f"Total scenarios in the JUnit reports: {total_scenarios_in_junit_reports}"
    )
    total_scenarios_in_console_output = get_total_scenarios_in_console_output(context)
    logging.info(
        f"Total scenarios in the console output: {total_scenarios_in_console_output}"
    )
    assert (
        total_scenarios_in_html_report
        == total_scenarios_in_junit_reports
        == total_scenarios_in_console_output
    ), f"Expected total scenarios to match, but found {total_scenarios_in_html_report} in the HTML report, {total_scenarios_in_junit_reports} in the JUnit reports, and {total_scenarios_in_console_output} in the console output"
...
def verify_total_scenarios_in_reports(context, consider_skipped_scenarios=True):
    ...
    total_scenarios_in_junit_reports = get_total_scenarios_in_junit_reports(
        context, consider_skipped_scenarios
    )
    logging.info(
        f"Total scenarios in the JUnit reports: {total_scenarios_in_junit_reports}"
    )
    assert (
        total_scenarios_in_html_report == total_scenarios_in_junit_reports
    ), f"Expected total scenarios to match, but found {total_scenarios_in_html_report} in the HTML report, {total_scenarios_in_junit_reports} in the JUnit reports"


@then(
    "I should see the same number of scenarios in the reports not considering the skipped scenarios"
)
def step_impl(context):
...
@then(
    'I should see the HTML report was generated and contains "{total_scenarios}" scenarios'
)
def verify_total_scenarios_in_html_report(
    context, total_scenarios=None, consider_skipped_scenarios=True
):
    total_scenarios_in_html_report = get_total_scenarios_in_html_report(context)
    logging.info(
        f"Total scenarios in the HTML report: {total_scenarios_in_html_report}"
    ...
    if total_scenarios is not None:
        assert total_scenarios_in_html_report == int(
            total_scenarios
        ), f"Expected the HTML report to contain {total_scenarios} scenarios, but found {total_scenarios_in_html_report}"
    else:
        assert (
            total_scenarios_in_html_report > 0
        ), "Expected the HTML report to be generated and contain scenarios"


@then('I should see the generated HTML report contains the "{string_to_search}" string')
def verify_string_in_html_report(
    context, string_to_search, string_should_be_present=True
):
    total_string_instances_in_html_report = get_string_instances_from_html_report(
        context, string_to_search
    )
    logging.info(
        f"Total instances of '{string_to_search}' in the HTML report: {total_string_instances_in_html_report}"
    )
    if string_should_be_present:
        assert (
            total_string_instances_in_html_report > 0
        ), f"Expected the HTML report to contain the string '{string_to_search}'"
    else:
        assert (
            total_string_instances_in_html_report == 0
        ), f"Expected the HTML report to not contain the string '{string_to_search}'"


@then(
    "I should see the generated HTML report does not contain internal BehaveX variables and tags"
)
def verify_string_not_in_html_report(context):
    ...
    for variable_or_tag in internal_behavex_variables_and_tags:
        total_string_instances_in_html_report = get_string_instances_from_html_report(
            context, variable_or_tag
        )
        logging.info(
            f"Total instances of '{variable_or_tag}' in the HTML report: {total_string_instances_in_html_report}"
        )
        assert (
            total_string_instances_in_html_report == 0
        ), f"Expected the HTML report to not contain the string '{variable_or_tag}'"
...
def get_total_scenarios_in_console_output(context):
    ...
    scenario_pattern = re.compile(
        r"(\d+) scenario.? passed, (\d+) failed, (\d+) skipped"
    )
        ...
        if file.endswith(".xml"):
            with open(os.path.join(junit_folder, file), "r") as file:
                ...
                if consider_skipped_scenarios:
                    total_scenarios_in_junit_reports += xml_content.count(
                        'status="skipped"'
                    )
