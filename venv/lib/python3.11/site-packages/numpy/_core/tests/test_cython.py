...
def test_npyiter_api(install_temp):
    ...
    assert checks.npyiter_has_index(it) == it.has_index is False

    it = np.nditer(arr, flags=["c_index"])
    assert checks.npyiter_has_index(it) == it.has_index is True
    assert checks.npyiter_has_delayed_bufalloc(it) == it.has_delayed_bufalloc is False

    it = np.nditer(arr, flags=["buffered", "delay_bufalloc"])
    assert checks.npyiter_has_delayed_bufalloc(it) == it.has_delayed_bufalloc is True

    it = np.nditer(arr, flags=["multi_index"])
    assert checks.get_npyiter_size(it) == it.itersize == np.prod(arr.shape)
    assert checks.npyiter_has_multi_index(it) == it.has_multi_index is True
