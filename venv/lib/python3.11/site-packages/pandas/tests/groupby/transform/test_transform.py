...
def test_transform_transformation_func(transformation_func):
    ...
    if transformation_func == "cumcount":
        ...
        def test_op(x):
            return x.transform("cumcount")

        def mock_op(x):
            return Series(range(len(x)), x.index)

    elif transformation_func == "fillna":

        def test_op(x):
            return x.transform("fillna", value=0)
        ...
        def test_op(x):
            return x.transform(transformation_func)

        def mock_op(x):
            return getattr(x, transformation_func)()
