...
class TestDataFrameMisc:
    ...
    def test_inplace_return_self(self):
        ...
        # set_index
        def f(x):
            return x.set_index("a", inplace=True)

        _check_f(data.copy(), f)

        # reset_index
        def f(x):
            return x.reset_index(inplace=True)
        ...
        # sort
        def f(x):
            return x.sort_values("b", inplace=True)

        _check_f(data.copy(), f)

        # sort_index
        def f(x):
            return x.sort_index(inplace=True)
        ...
        # replace
        def f(x):
            return x.replace(1, 0, inplace=True)

        _check_f(data.copy(), f)

        # rename
        def f(x):
            return x.rename({1: "foo"}, inplace=True)
        ...
        # reset_index
        def f(x):
            return x.reset_index(inplace=True, drop=True)
