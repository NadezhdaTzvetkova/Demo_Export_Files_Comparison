...
class CompDoc(object):
    ...
    def __init__(
        self, mem, logfile=sys.stdout, DEBUG=0, ignore_workbook_corruption=False
    ):
        self.logfile = logfile
        ...
        if DEBUG:
            ...
            print(
                "mem data: %d bytes == %d sectors" % (mem_data_len, mem_data_secs),
                file=logfile,
            )
            print(
        ...
        if MSATX_tot_secs == 0 and MSATX_first_sec_sid in (EOCSID, FREESID, 0):
            ...
            while sid not in (EOCSID, FREESID, MSATSID):
                ...
                if sid >= mem_data_secs:
                    msg = "MSAT extension: accessing sector %d but only %d in file" % (
                        sid,
                        mem_data_secs,
                    )
                    if DEBUG > 1:
        ...
        for msidx in xrange(len(MSAT)):
            ...
            if seen[msid]:
                raise CompDocError(
                    "MSAT extension corruption: seen[%d] == %d" % (msid, seen[msid])
                )
            seen[msid] = 2
    ...
    def _get_stream(
        self, mem, base, sat, sec_size, start_sid, size=None, name="", seen_id=None
    ):
        ...
        if size is None:
            ...
            while s >= 0:
                if seen_id is not None:
                    if self.seen[s]:
                        raise CompDocError(
                            "%s corruption: seen[%d] == %d" % (name, s, self.seen[s])
                        )
                    self.seen[s] = seen_id
    ...
    def _locate_stream(
        self, mem, base, sat, sec_size, start_sid, expected_stream_size, qname, seen_id
    ):
        ...
        while s >= 0:
            if self.seen[s]:
                if not self.ignore_workbook_corruption:
                    ...
                    dump_list(self.seen, 20, self.logfile)
                    raise CompDocError(
                        "%s corruption: seen[%d] == %d" % (qname, s, self.seen[s])
                    )
            self.seen[s] = seen_id
            tot_found += 1
