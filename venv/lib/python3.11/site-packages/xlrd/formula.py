...
def get_externsheet_local_range(bk, refx, blah=0):
    ...
    if not (0 <= ref_first_sheetx <= ref_last_sheetx < nsheets):
        if blah:
            print(
                "/// get_externsheet_local_range(refx=%d) -> %r" % (refx, info),
                file=bk.logfile,
            )
    ...
    while 0 <= pos < fmlalen:
        ...
        if opcode == 0x00:  # tArray
            ...
            if dodgy or shx1 < -1:
                ...
                if blah:
                    print(
                        "    tNameX: setting text to", repr(res.text), file=bk.logfile
                    )
            spush(res)
...
def rangename2drel(
    rlo_rhi_clo_chi, rlorel_rhirel_clorel_chirel, browx=None, bcolx=None, r1c1=0
):
