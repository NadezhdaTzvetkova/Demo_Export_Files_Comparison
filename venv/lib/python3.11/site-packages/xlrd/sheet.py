...
class Sheet(BaseObject):
    ...
    def read(self, bk):
        ...
        while 1:
            ...
            if rc == XL_NUMBER:
                ...
                self_put_cell(
                    rowx, colx, XL_CELL_TEXT, bk._sharedstrings[sstindex], xf_index
                )
                if do_sst_rich_text:
                    ...
                    if first_byte == 0:
                        ...
                        if not gotstring:
                            ...
                            if rc2 not in (XL_STRING, XL_STRING_B2):
                                raise XLRDError(
                                    "Expected STRING record; found 0x%04x" % rc2
                                )
                        ...
                        raise XLRDError(
                            "unexpected special case (0x%02x) in FORMULA" % first_byte
                        )
                else:
                    # it is a number
                ...
                if not fmt_info:
                    continue
                c = Colinfo()
                first_colx, last_colx, c.width, c.xf_index, flags = local_unpack(
                    "<HHHHH", data[:10]
                )
                ...
                if 0:
                    print("STANDARDWIDTH", self.standardwidth, file=self.logfile)
            elif rc == XL_GCW:
                if not fmt_info:
                ...
                if 0:
                    showgcw = (
                        "".join(map(lambda x: "F "[x], gcw)).rstrip().replace(" ", ".")
                    )
                ...
                if font_block:
                    (
                        ...
                        "<64x i i H H B 3x i 4x i i i 18x", data[pos : pos + 118]
                    )
                ...
                if bv < 80:
                    while pos < data_len:
                        self.horizontal_page_breaks.append(
                            (local_unpack("<H", data[pos : pos + 2])[0], 0, 255)
                        )
                        pos += 2
                else:
                    while pos < data_len:
                ...
                if bv < 80:
                    while pos < data_len:
                        self.vertical_page_breaks.append(
                            (local_unpack("<H", data[pos : pos + 2])[0], 0, 65535)
                        )
                        pos += 2
                else:
                    while pos < data_len:
    ...
    def fixed_BIFF2_xfindex(self, cell_attr, rowx, colx, true_xfx=None):
        ...
        if blah:
            fprintf(
                self.logfile, "New cell_attr %r at (%r, %r)\n", cell_attr, rowx, colx
            )
    ...
    def handle_hlink(self, data):
        ...
        if (options & 1) and not (
            ...
            if (
                ...
                if DEBUG:
                    fprintf(
                        self.logfile, "uplevels=%d shortpath=%r\n", uplevels, shortpath
                    )
                offset += nbytes
    ...
    def handle_obj(self, data):
        ...
        while pos < data_len:
            ...
            if ft == 0x15:  # ftCmo ... s/b first
                ...
                if OBJ_MSO_DEBUG:
                    fprintf(
                        self.logfile, "*** OBJ record has ft==0x0D 'notes' structure\n"
                    )
    ...
    def handle_txo(self, data):
        ...
        option_flags, o.rot, controlInfo, cchText, cbRuns, o.ifntEmpty = unpack(
            fmt, data[:fmtsize]
        )
        o.fmla = data[fmtsize:]
        upkbits(
