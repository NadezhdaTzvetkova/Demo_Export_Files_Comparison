...
def get_lfs_endpoint_url(git_repo, checkout_dir):
    try:
        with in_dir(checkout_dir):
            url = (
                check_output("git config -f .lfsconfig --get lfs.url".split())
                .strip()
                .decode("utf8")
            )
        ...
        with in_dir(git_repo):
            url = (
                check_output("git config --get remote.origin.url".split())
                .strip()
                .decode("utf8")
            )
    if url.endswith("/"):
...
def fetch(git_repo, checkout_dir=None, verbose=0):
    ...
    if checkout_dir == git_dir:
        print("Can't checkout into a bare repo, please provide a valid " "checkout_dir")
        raise SystemExit(1)
